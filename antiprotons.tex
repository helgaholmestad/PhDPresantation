  ...
  \vspace{-0.35cm}
      \begin{figure}
        \begin{tikzpicture}
          ...
          \node at (0,-3.5){\tiny{Figure by Markus Poessel}};
          \visible<2->{
  ...
  \begin{columns}
    \begin{column}{0.39\textwidth}
        ...
        \item{Building block of general relativity}
         ...
         %   }
         % \end{itemize}
          \uncover<3->{\item{Matter-antimatter asymmetry}}
          %\item{GPS}
      %\item{Test if the  weak equivalence principle holds also for antimatter}
      \end{itemize}
    \end{column}
...
\begin{frame}{\centering A classical moiré deflectometer}
  \only<1>{
  \begin{tikzpicture}
    \node at (0,0){\includegraphics[width=0.5\textwidth,angle=90]{fig/gratingsSideViewCropped.jpg}};
     \node at (5,0){Two of these gratings};
   
  \end{tikzpicture}
  }
  \scalebox{1.0}{\input{moire.tex}}
...
%        \item{Trade off between tagging efficiency and purity of sample}
  ...
  \item{Reconstruct the annihilation point}
    ...
    \uncover<5->{
    \item{Does a detector fulfilling these requirements already exist?}
      }
  \end{itemize}
  %%    \end{column}
  %%   \begin{column}{0.8\textwidth}
...
\end{frame}
\begin{frame}
  \scalebox{0.6}{\input{simulation.tex}}
\end{frame}
...
\begin{frame}{\centering Tagging efficiency}
  ...
  \uncover<3->{\item{A good compromise: At least 70 pixels and at least 1 prong}
    \begin{itemize}
    \item{Tagging efficiency 50 $\pm$ 10\%}
...
\begin{frame}{\centering Position resolution}
  \begin{columns}
    \begin{column}{0.5\textwidth}
      ...
      \end{itemize}}
      \uncover<3->{
      \item{Vertex fitting method excluding bad fits}
        \begin{itemize}
        \item{22\% of all clusters}
        \item{22~$\mu m$ resolution}
      \end{itemize}}
      \item<4->{Systematic uncertainty $ \approx \pm 1 \mu m$}
      \end{itemize}
    \end{column}
    \begin{column}{0.65\textwidth}
      \begin{figure}
